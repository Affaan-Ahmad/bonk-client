...
import { championIconSrc, formatPosition, phaseLabel, formatPhaseTimer } from "@/lib/league-helpers";
...
function TeamRow({
  ...
  return (
    ...
    >
      <span className="flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-md bg-black/40 text-xs font-semibold text-bonk-faint">
        {champion ? (
          <img src={championIconSrc(champion.id)} alt={champion.name} className="size-full object-cover" />
        ) : (
    ...
    >
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[230px_1fr_300px] gap-4">
        ...
        <aside className="flex min-h-0 flex-col gap-3 overflow-y-auto rounded-2xl border border-bonk-line bg-bonk-panel p-4 backdrop-blur-xl bonk-scroll">
          ...
          <BanStrip
            label="Team bans"
            bans={champSelectSession?.bans?.myTeamBans ?? []}
          ...
          <BanStrip
            label="Enemy bans"
            bans={champSelectSession?.bans?.theirTeamBans ?? []}
            championById={championById}
          />
        </aside>
        ...
        <aside className="flex min-h-0 flex-col gap-4 overflow-y-auto bonk-scroll">
          <div className="flex flex-col items-center gap-3 rounded-2xl border border-bonk-line bg-bonk-panel p-5 backdrop-blur-xl">
            ...
            <div className="flex size-28 items-center justify-center overflow-hidden rounded-2xl border border-bonk-line bg-black/40 font-display text-4xl font-bold text-bonk-faint">
              {selectedChampion ? (
                <img
